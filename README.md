...
### 2021.10.12 지도교수님 미팅
- 음성으로만 제어해야 하는 상황에서 사용할 수 있게 해보고 사용성이 어떤지
- 왜 꼭 음성으로 해야 하는가?
- 음성 명령이 필요한 상황을 어필하자

- 사용자마다 같은 의도에 대한 표현이 모두 다를 것이다. 그런 정보를 수집하자
- 사용자 테스트를 더 해보자
- 음성 명령어 개수를 늘리자, 다양화 하자
- 웹브라우저에서 가장 많이 사용하는 동작은 무엇인가?
- 그 중에서 어떤 것을 음성으로 지원하는 것이 좋은가?
- 중간보고서 작성 역할분담
    - 1~5 한상규
    - 6~10 김도연

### 2021.11.12 수업시간
- 최종 완성을 100%로 볼 때 현재 우리 조는 몇 %까지 와 있나?
    -	90%
- 남은 문제점은 무엇인가?
    - 잔 버그가 발견되는 중(탭 열거나 이동하다가 꺼지는 현상이 가끔 발생합니다.)
- 남은 문제점을 어떻게 공략할 것인가?
    - 지속적으로 보수하겠습니다.
### 2021.11.25 수업시간
![image](https://user-images.githubusercontent.com/31759313/145393415-1615595c-2641-4a03-8360-8a520b2cf137.png)
1. 지난주 보다 기본적으로 제공하는 음성 명령어 수가 늘어났습니다. 그리고 화자인식 기능에 프로필 등록할 때 녹음이 되고 있다는 표시를 버튼을 빨간색으로 표시하여 명시적으로 나타내게 함

2. 100%이나 계속해서 디테일을 향상시켜보겠습니다.

3,4. 없음

5. 딱히 추가적으로 고려할 사항이 없습니다. 시연할때 아주 쉽게 할 수 있을 것입니다.

### 2021.12.02 수업시간
교수님께서 이미 프로젝트에 대해 잘 아시기 때문에 이미 알고 계시는 내용은 최소화 하고 취약점을 개선해서 잘 동작한다고 말씀드리자.
10분 발표

- 시연 계획:
    - 손을 사용할수 없는 요리 중 레시피 찾는 시나리오 – 이 때 음성 제어 기능을 소개한다.
    - 사용자 지정 명령어 기능을 소개한다. – 검색 기능을 추가하는 장면 시연, 코드를 직접 입력할 수 있다고 설명만 하기
    - 화자 인식 – 등록하는 것은 할 수 있다고만 말하고, accept, reject 나는 모습만 시연하기 
    - ‘오미크론 현재현황 검색해줘’
    - 화자 인식 프로필 등록, 사용자 지정 명령어 만들기 시연은 녹화해서 보여드리기
